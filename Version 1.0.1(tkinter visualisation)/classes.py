...
class League:
    ...
    def __get_teams(self):
        ...
        for team in teams_js['teams']:
            teams[team['name']] = Team(team['name'], self.matches,
                                       self.matchday, self._standings)
        return teams
    ...
    def print_standings(self):
        ...
        with open("data/" + self.code + "standings.json", "r", encoding="utf-8") as file:
            standings_js = json.load(file)
        standings = ""
        standings += standings_js['competition']['name'] + "\n"
        standings += "№ " + " Name" + (24*" ") + "G  W  D  L  GF  GA  GD   P" + "\n"
        for team in standings_js['standings'][0]['table']:
            standings += (2-len(str(team['position'])))*" " + str(team['position']) + " " + team['team']['name'] +\
    ...
    def print_scorers(self):
        ...
        for player in scorers['scorers']:
            result += " "*(2 - len(str(num))) + str(num) + "." + player['player']['name'] +\
...
class Team:
    ...
    def __init__(self, name, match_list, matchday, standings):
        self.name = name
